...
| ---------- | -----------: | ----------------------: | --------------------: | ----------------: | ---------------: | ----------: | -------------: |
| Legacy     |            9 |                       1 |                    10 |          1,491 ms |     869–1,643 ms |    7,499 ms | 6,459–9,515 ms |
| Refactored |           15 |                       0 |                    15 |            280 ms |       172–461 ms |    6,494 ms | 3,450–9,521 ms |

The legacy suite contains **9 test methods** across 2 classes, plus one `@TestSetup` execution, so Salesforce reports `testsRan: 10`. The refactored suite contains **15 test methods** across 4 classes and no `@TestSetup`, so Salesforce reports `testsRan: 15`. Salesforce's `testExecutionTime` includes the legacy setup execution; only the displayed method count excludes it. The raw number of tests is not a measure of equivalent work: one legacy `convertTest` contains several scenarios and crosses shared setup, SOQL, metadata, and DML, while one isolated helper test may execute only an in-memory transformation. Test count is nevertheless meaningful context here because the refactored suite executes five more Salesforce-reported test units and still has the lower platform time.

In this sample, the refactored median was **81.2% lower** and **5.3× faster** in Salesforce execution time. Its wall-time median was **13.4% lower**, but wall results are noisier because CLI startup, network latency, queueing, and result processing dominate short test runs. The result demonstrates the feedback-speed benefit of isolated tests rather than promising an identical ratio in every org or run.

The current results are preserved in the [legacy summary](benchmark-results/legacy-20260818-083909/summary.md), [legacy CSV](benchmark-results/legacy-20260818-083909/summary.csv), [legacy raw JSON](benchmark-results/legacy-20260818-083909/raw/), [refactored summary](benchmark-results/refactored-20260818-083953/summary.md), [refactored CSV](benchmark-results/refactored-20260818-083953/summary.csv), and [refactored raw JSON](benchmark-results/refactored-20260818-083953/raw/).
